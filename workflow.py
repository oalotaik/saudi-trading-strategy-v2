...
from portfolio import load_state, save_state, mark_to_market, manage_positions, select_candidates, build_action_list, apply_entries, summarize_state

# --- Progress formatting to match other modules ---
from rich.progress import (
    Progress, SpinnerColumn, TextColumn, BarColumn, MofNCompleteColumn, TimeElapsedColumn, TimeRemainingColumn,
)
...
def weekend_refresh(universe: list, no_progress: bool = False):
    ...
    if not ind:
        error("No data for weekend refresh."); return

    idx = dfetch.get_index_history(lookback_days=1500, refresh=False)
    idx_ind = compute_indicators(idx)
    breadth = 100.0 * np.mean([float(df.iloc[-1]["Close"] > df.iloc[-1]["EMA50"]) for df in ind.values()])
    cond1 = (idx_ind.iloc[-1]["Close"] > idx_ind.iloc[-1]["SMA200"]) and (idx_ind["SMA200"].iloc[-1] > idx_ind["SMA200"].iloc[-20])
    ...
    fund_raw = {t: dfetch.get_fundamentals_cached_or_fetch(t, compute_fundamental_metrics, manual_df) for t in universe if t in ind}
    fs_map = sector_relative_scores(fund_raw, sectors)

    # Sector strength 20d RS vs index
    idx_ret20 = idx_ind["Close"].pct_change(20).iloc[-1]
    sec_returns = {}
    for sec in set(sectors.values()):
        members = [t for t, s in sectors.items() if s == sec and t in ind and len(ind[t]) >= 25]
        if not members: continue
        rlist = [ind[t]["Close"].pct_change(20).iloc[-1] for t in members]
        rlist = [r for r in rlist if pd.notna(r)]
        if rlist:
            sec_returns[sec] = float(np.nanmean(rlist) - (idx_ret20 if pd.notna(idx_ret20) else 0.0))
    sec_pct = (pd.Series(sec_returns).rank(pct=True) * 100.0).to_dict() if sec_returns else {}
    ...
    for t in universe:
        ...
        rows.append([fmt_ticker_and_name(t, names.get(t)), sectors.get(t,"Unknown"), round(ts,2), round(fs,2), round(sc,2), round(comp,2)])
    rows.sort(key=lambda r: r[-1], reverse=True)

    print_panel("Market Regime (Weekend)", f"Score: {regime_score}/3 | Breadth SMA50: {breadth:.1f}%")
    print_table("Watchlist (pre-ranked by CompositeRank)", ["Ticker — Name","Sector","Tech","Fund","Sector","Composite"], rows[:25])

def daily_after_close(universe: list, initial_equity: float = 100000.0, no_progress: bool = False):
    info("Daily workflow: update technicals, triggers, select, size, and manage live positions.")
    ...
    with _make_progress(disabled=no_progress) as progress:
        ...
        for t in universe:
            ...
            progress.advance(t_prices)
    if not ind:
        error("No price/indicator data today."); return
    ...
    idx = dfetch.get_index_history(lookback_days=1500, refresh=False)
    idx_ind = compute_indicators(idx)
    idx_ret20 = idx_ind["Close"].pct_change(20).iloc[-1] if len(idx_ind) >= 21 else np.nan
    ...
    sec_pct = (pd.Series(sec_returns).rank(pct=True) * 100.0).to_dict() if sec_returns else {s:50.0 for s in unique_secs}

    rs_pct_within = {}
    for sec in unique_secs:
        members = [t for t, s in sectors.items() if s == sec and t in ind and len(ind[t]) >= 25]
        if not members: continue
        ser = pd.Series({t: ind[t]["Close"].pct_change(20).iloc[-1] for t in members}).dropna()
    ...
    selection_pool = {t: ind[t] for t in ind if liq.get(t, False) and tech.get(t, {}).get("PostureUptrend", False)}

    # Fundamentals: cached/static (ignore manual overrides to mirror backtest default)
    fund_raw = {t: dfetch.get_fundamentals_cached_or_fetch(t, compute_fundamental_metrics, None) for t in selection_pool.keys()}
    fs_map = sector_relative_scores(fund_raw, sectors)
    fs_pct = {}
    sec_groups = {}
    ...
    for t in selection_pool:
        sub = ind[t]
        triggers[t] = {"D1": bool(trigger_D1(sub)), "D2": bool(trigger_D2(sub)), "DB55": bool(trigger_DB55(sub))}

    candidates = select_candidates(selection_pool, sectors, triggers, sec_pct, rs_pct_within)
    ...
    if dd <= -config.PORTFOLIO_MAX_DRAWDOWN and state.positions:
        for t, p in list(state.positions.items()):
            ...
            trades_today.append({"Date": dt.strftime("%Y-%m-%d"), "Ticker": t, "Action": "SELL", "Price": px, "Shares": p.shares, "Reason": "MAX_DD"})
    ...
    breadth = 100.0 * np.mean([float(df.iloc[-1]["Close"] > df.iloc[-1]["EMA50"]) for df in ind.values()])
    i = len(idx_ind) - 1
    slope = (idx_ind["SMA200"].iloc[i] - idx_ind["SMA200"].iloc[i - config.REGIME_SLOPE_WINDOW]) if i >= config.REGIME_SLOPE_WINDOW else 0.0
    ...
    if state.cooldown > 0:
        ...
        entry_actions, mgmt_actions = build_action_list(state, selection_pool, candidates, sectors, equity_for_sizing=equity, regime_score=regime_score)
    ...
    print_table("Portfolio Status", ["Ticker — Name","Close","Entry","Stop","Trail","Shares","Sector"], rows_state)

    next_actions = []
    for a in entry_actions:
        nm = names.get(a['Ticker'],''); disp = f"{a['Ticker']} — {nm}" if nm else a['Ticker']
        next_actions.append([disp, a['Action'], f"{a['Price']:.2f}", a['Shares'], f"Stop={a['Stop']:.2f}"])
    ...
    print_table("Next-Day Action List", ["Ticker — Name","What","Price/To","Shares","Notes"], next_actions)
...
def monthly_dashboard():
    ...
    try:
        ...
        print_panel("Dashboard", "No equity history yet."); return
    eq["Ret"] = eq["Equity"].pct_change()
    cagr = (eq["Equity"].iloc[-1] / eq["Equity"].iloc[0]) ** (252.0 / max(len(eq)-1,1)) - 1
...
def build_parser():
    ...
    p.add_argument("--equity", type=float, default=100000.0, help="Initial equity if portfolio is new")
...
def main(args):
    ...
    if args.daily:
        daily_after_close(universe, initial_equity=args.equity, no_progress=args.no_progress)
    if args.monthly:
        monthly_dashboard()
